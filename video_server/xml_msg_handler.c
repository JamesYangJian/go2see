...
int program_request_notify_handler(void *pChannel, ezxml_t pxml)
{
    ...
    if (result == M_PG_REQ_NOTIFY_RET_SUCCEED)
    {
        ...
        else
        {
            ...
            memset(szPGFmt, 0, sizeof(szPGFmt));
            memset(szTransProt, 0, sizeof(szTransProt));
            memset(szStreamProt, 0, sizeof(szStreamProt));
            sscanf(pPgTransferInfo->szProtoType, "%s/%s/%s", szPGFmt, szStreamProt, szTransProt);
            if (strcmp(szTransProt, "udp") == 0)
            {
