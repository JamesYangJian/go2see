...
void connect_to_video_server()
{
    ...
    tv.tv_sec = 2;
    tv.tv_usec = 0;
    iRet = tl_connect(video_server_sock, ntohl(inet_addr(pVideoSvrIP)), (unsigned short)iVideoSvrPT, &tv);
    if (iRet < 0)
    {
    ...
    gp_video_server_channel->channel_type = E_SERVER_CHANNEL;
        ...
        if (iRet > 0)
        {
            if (FD_ISSET(g_sys_msg_sock, &rd_set))
            {
                ...
                if (new_sock > 0)
                {
                    ...
                    {
                        pChn->sock = new_sock;
                        pChn->channel_type = E_CLIENT_CHANNEL;
                    }
                    else
                    {
                        close(new_sock);
